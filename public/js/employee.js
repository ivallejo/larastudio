...
$('.a-register').click( function() {
  if( inputDocument.value != '' && inputFirstName.value != '' && inputLastName.value != '' && inputPassword.value != '' ) {
    if(inputPassword.value != inputPasswordConfirm.value) {
      let notifier = new Notifier();
      notifier.info('Las contraseñas no coinciden.', 'Atencion!');
      return
    }
    const data = {
      id: 0,
    ...
    $.ajax({
      ...
      success: async function (resp) {
        let notifier = new Notifier();
        notifier.success('Empleado registrado con éxito.', 'Empleado registrado!');
...
if( customerValOld != customerValNew ) {
  $.ajax({
    ...
    success: async function (resp) {
      let notifier = new Notifier();
      notifier.success('Campo actualizado con éxito.', 'Empleado Actualizado!');
    },
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      console.log(XMLHttpRequest);
    },
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      $.ajax({
        ...
        success: async function (resp) {
          let notifier = new Notifier();
          notifier.success('Empleado eliminado con éxito.', 'Empleado Eliminado!');
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
          console.log(XMLHttpRequest);
        },
